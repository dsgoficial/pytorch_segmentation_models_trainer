...
const INITIAL_VAL_DATASET = {
  ...
  augmentations: VAL_AUG_DEFAULTS,
}
...
// ── Serialização para o formato YAML do framework ────────────────────────────
...
function serializeAugmentations(augmentations) {
  // params já estão em snake_case (vindos do schema) — apenas espalhamos
  const result = augmentations.map(aug => ({
    _target_: aug.target,
    always_apply: false,
    ...aug.params,
  }))
  // Normalize e ToTensorV2 são sempre os últimos, fixos
  result.push({
...
// ── Deserialização (YAML → estado interno) ────────────────────────────────────
  ...
  return (
    <div style={styles.app}>
      <header style={styles.header}>
        <h1>PyTorch Segmentation Models Trainer — Config Builder</h1>
...
const styles = {
  ...
  header:     { background: '#fff', borderBottom: '1px solid #e5e5e5', padding: '12px 24px', position: 'sticky', top: 0, zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  ...
  main:       { display: 'grid', gridTemplateColumns: '1fr 1fr', flex: 1, height: 'calc(100vh - 53px)' },
