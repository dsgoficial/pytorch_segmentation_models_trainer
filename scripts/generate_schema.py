...
_ALBUMENTATIONS_EXCLUDE = {
    "BasicTransform", "DualTransform", "ImageOnlyTransform",
    "NoOp", "Compose", "ReplayCompose", "OneOf", "SomeOf",
    "Sequential", "BaseCompose", "SelectiveChannelTransform",
}
...
def _get_param_default(param):
    ...
    if isinstance(val, (list, tuple)):
        return list(val) if all(isinstance(x, (int, float, str, bool)) for x in val) else None
    return None
...
_TORCHMETRICS_SEGMENTATION = [
    ...
    "Specificity", "AUROC", "AveragePrecision", "CohenKappa",
    "MatthewsCorrCoef", "ConfusionMatrix",
]
...
def main():
    ...
    if libs_ok == 0:
        print("[ERROR] No libraries found. Install requirements first.", file=sys.stderr)
        sys.exit(1)
