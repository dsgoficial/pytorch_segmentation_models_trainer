...
## Mean-Teacher AIO2 Baseline (Liu et al., TGRS 2024)

- New noisy-label training baseline, cross-checked against the [official reference implementation](https://github.com/zhu-xlab/AIO2) (not the paper text alone — several details, like the ACT window sizes and the checkpoint/resume design, are only visible in the code):
  - `utils/act_trigger.py`: `ACTTracker` — Adaptive Correction Trigger, a near-verbatim port of `early_learning_detection.py`. Detects, without ground truth, the epoch to stop warm-up and start online correction, via sliding-window local linear regression + exponential-saturation curve fit on the teacher's training accuracy. Default `window_sizes=[10, 20, 30, 40]` matches the official README's published example; the buffer `z` from the paper's §2.3 is **not a free hyperparameter** — the official code derives it as `mean(window_sizes)`.
  - `utils/o2c_correction.py`: `online_object_correction` — Online Object-wise label Correction (O2C), generalized to multiclass (the official implementation is binary-only, building footprints, one foreground class — there is no multiclass version in the paper or the code). Per class in `classes_to_correct`, finds connected components of the teacher's prediction that don't overlap the noisy label's own pixels of that class and proposes adding them. Two decisions needed only because of the multiclass generalization (both documented in the docstring and the article repo's decision log, neither has support from the original paper): a configurable `conflict_resolution` (`priority_order` or `teacher_confidence`) for when two classes propose the same pixel, and a "fixed classes" restore step — replicated verbatim from `tools.region_correction.correction.apply_region_correction` (SAM/SLICO) — that protects any pixel whose *original* class isn't in `classes_to_correct`, regardless of what was proposed for it (the first draft of this function got this backwards, gating eligibility by which class could *propose* rather than which pixel could *change*; caught before landing). `filter_size` erodes new-object boundaries instead of emitting the official code's continuous soft labels, since `WeightedDiceCrossEntropyLoss` needs a hard target.
  - `custom_models/mean_teacher_wrapper.py`: `MeanTeacherWrapper` (plain `nn.Module`, mirrors the `BaseDomainAdaptationMethod` domain-logic/LightningModule split) — student + EMA teacher pair, same decay-warmup formula as `EMACallback`. `ema_update` only touches `.parameters()`, never BatchNorm running-stat buffers, matching a property of the official `update_ema_variables` (documented, not silently "fixed").
  - `model_loader/mean_teacher_model.py`: `MeanTeacherModel(Model)` — orchestrates both phases. **Two-phase, two `Trainer.fit()` calls**, mirroring the official code's two CLI invocations (`--resume`/`--resume_from_detection`) rather than an in-run weight rewind: phase 1 (warm-up) tracks the teacher's train IoU, checkpoints periodically, and self-stops (`trainer.should_stop = True`) the epoch ACT triggers; phase 2 (`model.o2c_active = True`, reload via `Trainer.fit(model, ckpt_path=...)` — native optimizer-state restore) applies O2C every training step. `find_nearest_checkpoint` picks the nearest warm-up checkpoint at or before the trigger epoch for the phase-1 → phase-2 handoff. Simplification vs. the plan first drafted for this baseline: no separate `ACTCheckpointCallback` — ACT needs the *teacher's* training accuracy specifically (the base `Model` class's own metrics reflect the student), which only this LightningModule can compute directly; a generic callback would need just as much coupling to model internals for no real separation of concerns.
  - Loss: reuses `WeightedDiceCrossEntropyLoss` (already existed) — no new loss class for this baseline, unlike Zhu2019.
  - `MeanTeacherModel`: new `correct_base` config (`"iter"` default, or `"epoch"`) — matches the official code's other supported correction-timing mode, independent of `correct_model` (which network) and orthogonal to it. `"iter"` recomputes the correction source every training step from the live network state (true "online", nothing cached). `"epoch"` takes one frozen, no-grad snapshot of the source network per epoch (`on_train_epoch_start`) and reuses it for every batch that epoch, amortizing the forward + connected-component cost from per-batch to per-epoch × unique images. Caching *across* runs was considered and rejected — the correction is a function of that run's specific teacher trajectory (seed/batch-order/EMA history dependent), so a cached correction from one run is invalid for another, unlike the SAM/SLICO mask cache elsewhere in this project (SAM is fixed throughout training; the AIO2 teacher co-evolves with it, so there's no stable cross-run cache key). Discussion recorded in `website/docs/user-guide/mean-teacher-aio2.md` ("When to cache").
- `conf/examples/mean_teacher_aio2.yaml`, `website/docs/user-guide/mean-teacher-aio2.md` (incl. a caution box: AIO2 is designed for *incomplete* label noise — missing objects — not the geometric boundary-misclassification noise in this project's data; kept as a baseline anyway since showing a false-negative-oriented SOTA method doesn't solve a different noise type is itself a useful result, but flagged so it isn't presented as a like-for-like comparison).
- Also added `conf/examples/zhu2019_boundary_relaxation.yaml`, missed when `BoundaryLabelRelaxationLoss` first landed.
- Tests: `tests/test_act_trigger.py`, `tests/test_o2c_correction.py`, `tests/test_mean_teacher_wrapper.py`, `tests/test_mean_teacher_model.py` (24 + 19 + 19 + 47 = 109 tests). 100% coverage on all four new modules.
- Design decisions (conflict resolution needing a pilot, the noise-type mismatch, the fixed-classes fix) are also logged in the article repository (`doutorado_artigo_sam_noisy_labels/descricao_novos_experimentos.md`, "Adendo" section) as they were made, for traceability back to the paper's methodology writeup.

## Boundary Label Relaxation Loss (Zhu et al., CVPR 2019)

- New `custom_losses/boundary_relaxation_loss.py`: `BoundaryLabelRelaxationLoss`, the label-relaxation component of Zhu, Y. et al., "Improving Semantic Segmentation via Video Propagation and Label Relaxation," CVPR 2019 (video propagation, the paper's other contribution, is out of scope — no video sequences in this project's remote-sensing data). For each pixel, instead of maximizing the probability of its single annotated class, the loss maximizes the probability mass assigned to the *union* of classes present in its hard label's 3×3 neighbourhood: `L = -log(Σ_{C∈N} P(C))`. On interior pixels (all 3×3 neighbours share the centre's class, `|N|=1`) this is an exact algebraic reduction to standard cross-entropy — not a separate loss blended in — so it's a strict generalization, verified by a dedicated test comparing against `F.cross_entropy` bit-for-bit within tolerance.
- Neighbourhood membership is computed with a single `F.max_pool2d(one_hot_label, kernel_size=3, stride=1, padding=1)` call — no per-pixel Python loop or `F.unfold` needed. `ignore_index` pixels (default 255, matching `WeightedDiceCrossEntropyLoss`/`Model._soft_to_hard_masks` convention elsewhere in this project) are excluded both from the loss mean and, by zeroing their one-hot row before the max-pool, as a neighbour-class source for adjacent valid pixels — an ignored region can't leak a spurious "relaxed" class into a valid neighbour's target set.
- No new hyperparameter beyond the (fixed, 3×3) neighbourhood window, matching the original paper — only `num_classes` and `ignore_index` are configurable.
- Registered as `BoundaryLabelRelaxationLossConfig` in `config_definitions/loss_config_definition.py` (`group="loss", name="boundary_relaxation_loss"`), plugs in via the existing simple-loss path (`cfg.loss` → `instantiate`) — no `Model` subclass required, works directly against the hard-label batches already produced by TVS-style datasets.
- Tests: `tests/test_boundary_relaxation_loss.py` (new, 15 tests) — output contract, gradient flow, exact CE-reduction on interior-only labels, relaxation actually lowers loss vs. plain CE on a checkerboard border case, `ignore_index` handling (incl. all-pixels-ignored edge case), custom `mask_key`/`ignore_index`, and the inherited `Loss.forward()` wrapper contract.
- Docs: `website/docs/user-guide/boundary-label-relaxation.md`.

## SAM/SLICO: pluggable image source, SLICO threading, SAM prefetch

- New `tools/mbtiles/image_source.py`: `mbtiles_path` (SAM and SLICO) and each entry of `lulc_paths` now accept four forms — a single file path (existing behavior, unchanged — MBTiles, VRT, or any rasterio-readable raster); a directory matched by spatial bounds (`{directory, extensions, recursive, match_by: "bounds"}`, the default); a directory matched by filename basename against the mask tile (`match_by: "basename"` — no file is opened to build the index, just listed); or a CSV manifest, one row per tile (`{csv_path, path_column, mask_path_column, default_crs}`) with a required `path` column, an optional `mask_path` column for direct tile-keyed lookup, and optional `minx,miny,maxx,maxy[,crs]` columns for bounds-based fallback — mirrors the wide `tile_id,image_path,mask_path,<lulc columns>` CSV convention already used by `tools.soft_labels`. `resolve_image_source()` dispatches (dict discriminated by `directory` vs `csv_path` key); all three multi-file forms resolve to `TiledImageSource`, whose `resolve_candidates()` tries a direct tile-keyed lookup first (basename or CSV `mask_path` — an O(1) dict lookup, no spatial computation) before falling back to bounds-based search. `read_source_aligned_to_mask_window` (`tools/mbtiles/alignment.py`) composites the resolved candidate(s) into the destination window (first file with data wins per pixel) — no new dependency (no GDAL/osgeo bindings in this project; built on `rasterio.vrt.WarpedVRT`, reusing the existing single-file warp path per candidate).
- SLICO: new `n_workers` config field — tiles are processed concurrently via `ThreadPoolExecutor` when `>1` (default 1 = unchanged sequential behavior). Parallelism is at **tile** granularity, not chunk: each tile writes to its own output file, so there's no shared-file write contention to lock around. `skimage.segmentation.slic`'s inner loop is Cython and releases the GIL, as does rasterio I/O, so this is real wall-clock parallelism, not just concurrency. Tiles are ordered largest-estimated-chunk-count first (`_estimate_n_chunks`) before dispatch, for load balancing across uneven per-tile chunk counts — same rationale as the chunk-level `ProcessPoolExecutor` precedent in `doutorado_disciplina_deep_learning_artigo/02-experimentos/scripts/build_sam_corrected_knn_dataset.py`, adapted to tile granularity here because that script's chunk-level stage never wrote to a shared output file (ours does, so tile-level is the safe granularity). Verified with a real (unmocked) test: threaded and sequential runs produce byte-identical output masks (`tests/test_slico_label_corrector_integration.py`).
- SAM: new `points_per_batch` config field (default 64, matches `SamAutomaticMaskGenerator`'s own default) — exposes AMG's internal point-prompt batching, previously silently defaulted. New `prefetch` config field (default `True`): the next chunk's aligned imagery/LULC read runs on a background thread while SAM processes the current chunk, overlapping I/O with GPU compute. This is not cross-image batching — SAM's public API (`SamAutomaticMaskGenerator.generate()`) still processes one image per call, so GPU compute time is unchanged; prefetch only hides the I/O wait between calls. `SamLabelCorrector._process_chunk` was split into `_read_chunk_inputs` (I/O, opens its own dataset handles — safe to call from a background thread, no handle shared with the caller) and `_run_sam_and_correct` (cache lookup/SAM inference/correction/write, stays on the calling thread since CUDA context and the GIL make cross-thread GPU calls pointless).
- Tests: `tests/test_image_source.py`, `tests/test_alignment_tiled_source.py` (new); `tests/test_sam_label_corrector.py`, `tests/test_slico_label_corrector.py` extended for the new config fields and restructured internals; `tests/test_slico_label_corrector_integration.py` (new, unmocked end-to-end threaded-vs-sequential equivalence check). 100% coverage on all touched/new modules.
- **Renamed** `include_bags` → `include_base_mask`, and `apply_region_correction`/`apply_sam_correction`'s `bags_raw` parameter → `base_mask`, in the shared core and both SAM/SLICO configs. The old names assumed the mask being corrected was always the specific topographic-vector-source ("BAGS") dataset from the original experiment; `masks_dir` can point at any mask, so the API shouldn't bake in that assumption. Pure rename — no behavior change, values/algorithm unaffected.
