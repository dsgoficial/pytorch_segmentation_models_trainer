...
from pytorch_segmentation_models_trainer.predict import (
    instantiate_inference_processor, instantiate_model_from_checkpoint,
    instantiate_polygonizer, predict)
...
class Test_TestPredict(unittest.TestCase):
    def setUp(self):
        ...
        self.checkpoint_file_path = self.get_checkpoint_file('frame_field_resnet152_unet_200_epochs.ckpt')
    ...
    def test_run_predict_from_object(self, config_name: str) -> None:
        with initialize(config_path="./test_configs"):
            ...
            for i in range(cfg.inference_image_reader.n_first_rows_to_read):
                ...
                assert os.path.isfile(os.path.join(self.output_dir, f'seg_{name}_inference.tif'))
                assert os.path.isfile(os.path.join(self.output_dir, f'crossfield_{name}_inference.tif'))
