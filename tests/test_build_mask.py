...
from pytorch_segmentation_models_trainer.tools.mask_building.mask_builder import (
    MaskBuilder, build_destination_dirs, merge_csv_datasets)
...
class Test_TestBuildMask(unittest.TestCase):
    ...
    def test_merge_csv_datasets(self):
        ds_csv1 = os.path.join(root_dir, 'data', 'build_masks_data', 'dsg_dataset_only_polygon_masks.csv')
        ds_csv2 = os.path.join(root_dir, 'data', 'build_masks_data', 'dsg_dataset_other_masks.csv')
        output_csv = os.path.join(self.output_dir, 'dsg_dataset_merged.csv')
        merge_csv_datasets(ds_csv1, ds_csv2, 'image', output_csv)
        expected_df = pd.read_csv(os.path.join(root_dir, 'expected_outputs', 'build_masks', 'dsg_dataset.csv')).sort_values('image')
    ...
    def test_build_masks_with_bounding_boxes(self):
        with initialize(config_path="./test_configs"):
            ...
            print(cfg)
            csv_output = build_masks(cfg)
        expected_df = pd.read_csv(os.path.join(expected_output_path, 'dsg_dataset_with_bboxes.csv')).sort_values('image')
        output_df = pd.read_csv(csv_output).sort_values('image')
        pd.testing.assert_frame_equal(
            expected_df.reset_index(drop=True),
            output_df.reset_index(drop=True)
