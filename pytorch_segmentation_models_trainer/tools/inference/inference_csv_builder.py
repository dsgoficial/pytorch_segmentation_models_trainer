...
class InferenceCSVBuilder:
    ...
    def build_csv(
        self, output_csv_path: str, validate_images: bool = True
    ) -> pd.DataFrame:
        """
        ...
        if self.masks_folder:
            ...
            logger.info(
                f"  Masks found: {masks_found} ({masks_found/len(df)*100:.1f}%)"
            )
    ...
    def load_or_build_csv(
        self, output_csv_path: str, force_rebuild: bool = False
    ) -> pd.DataFrame:
        """
...
def build_inference_csv_from_config(config) -> str:
    ...
    if "output_csv_path" in config and config.output_csv_path:
        ...
        folder_hash = hashlib.md5(config.images_folder.encode()).hexdigest()[:8]

        timestamp = str(int(time.time() * 1000))  # milliseconds

        # Gerar path com identificador único
        output_csv_path = os.path.join(
            config.images_folder, f"inference_dataset_{folder_hash}_{timestamp}.csv"
        )
