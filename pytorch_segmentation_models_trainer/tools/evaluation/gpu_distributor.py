...
class GPUDistributor:
    ...
    def assign_experiments(
        self, experiments: List[DictConfig]
        ...
        if self.strategy == "one_experiment_per_gpu":
            ...
            logger.warning(
                f"Unknown strategy: {self.strategy}. " f"Falling back to sequential."
            )
            return self._assign_sequential(experiments)
    ...
    def _assign_one_per_gpu(
        ...
        for idx, exp in enumerate(experiments):
            ...
            logger.info(f"Assigned experiment '{exp.name}' to GPU {gpu_id}")
    ...
    def _assign_sequential(
        ...
        logger.info(f"All {len(experiments)} experiments will run on {device_name}")

        return {device_id: experiments}

    def get_device_for_experiment(
        self, experiment_name: str, assignments: Dict[int, List[DictConfig]]
    ) -> int:
        """
    ...
    def wait_for_gpu_memory(
        self, gpu_id: int, required_gb: float, timeout_seconds: int = 300
    ) -> bool:
        """
        ...
        logger.info(f"Waiting for GPU {gpu_id} to have {required_gb:.1f}GB free...")
        ...
        while time.time() - start_time < timeout_seconds:
            ...
            if mem_info["free_gb"] >= required_gb:
                logger.info(f"GPU {gpu_id} now has {mem_info['free_gb']:.1f}GB free")
                return True
