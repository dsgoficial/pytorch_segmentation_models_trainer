...
class DirectFolderEvaluator:
    ...
    def __init__(
        ...
        if not self.gt_folder.exists():
            raise FileNotFoundError(
                f"Ground truth folder not found: {ground_truth_folder}"
            )
    ...
    def build_matching_pairs(self) -> List[Dict[str, str]]:
        ...
        if len(gt_files) == 0:
            raise ValueError(
                f"No ground truth files found with pattern {self.gt_pattern}"
            )
        ...
        if not_found:
            logger.warning(
                f"Could not find predictions for {len(not_found)} ground truth files:"
            )
