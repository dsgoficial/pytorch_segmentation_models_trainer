...
class EvaluationPipeline:
    ...
    def run(self) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Total time: {elapsed_time:.2f}s ({elapsed_time/60:.2f} minutes)"
            )
            logger.info("=" * 80)
    ...
    def _prepare_dataset(self) -> str:
        ...
        if (
            hasattr(self.config.evaluation_dataset, "direct_folder_evaluation")
            and self.config.evaluation_dataset.direct_folder_evaluation.enabled
        ):
            ...
            csv_path = os.path.join(
                self.config.output.base_dir, "direct_evaluation_dataset.csv"
            )
        ...
        if self.config.evaluation_dataset.build_csv_from_folders.enabled:
            ...
            csv_path = os.path.join(
                self.config.output.base_dir, "generated_dataset.csv"
            )
    ...
    def _run_predictions(self, dataset_csv: str) -> Dict:
        ...
        if (
            hasattr(self.config.pipeline_options, "load_predictions_from_folder")
            and self.config.pipeline_options.load_predictions_from_folder.enabled
        ):
            return self._load_existing_predictions()

        # Comportamento original: executar predições
        if (
            self.config.pipeline_options.parallel_inference.enabled
            and self.gpu_distributor is not None
        ):
    ...
    def _load_existing_predictions(self) -> Dict:
        ...
        if (
            ...
            and self.config.pipeline_options.load_predictions_from_folder.base_folder
        ):
            base_folder = (
                self.config.pipeline_options.load_predictions_from_folder.base_folder
            )
        ...
        for exp in self.experiments:
            ...
            if (
                hasattr(exp, "precomputed_predictions_folder")
                and exp.precomputed_predictions_folder
            ):
        ...
        # Resumo
        loaded_count = sum(
            1 for p in predictions_info.values() if p.get("loaded", False)
        )
    ...
    def _validate_predictions_folder(self, folder: str, experiment_name: str) -> Dict:
        """
        ...
        return {"valid": True, "num_files": len(valid_files), "error": None}
    ...
    def _run_predictions_sequential(self, dataset_csv: str) -> Dict:
        ...
        for exp in self.experiments:
            ...
            try:
                ...
                logger.error(
                    f"Failed to run prediction for {exp.name}: {e}", exc_info=True
                )
                raise
    ...
    def _run_single_prediction(
        self, experiment: DictConfig, dataset_csv: str, gpu_id: int = -1
    ):
        """
        ...
        if not os.path.exists(script_path):
            raise FileNotFoundError(f"predict.py not found at {script_path}")

        cmd = [
            sys.executable,
            script_path,
            "--config-dir",
            str(Path(experiment.predict_config).parent),
            "--config-name",
            Path(experiment.predict_config).stem,
        ] + overrides
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(
                f"Prediction failed for {experiment.name}. " f"Check logs for details."
            )
    ...
    def _generate_visualizations(self, aggregated: Dict):
        ...
        if self.config.visualization.confusion_matrix.save_comparison:
            ...
            self.confusion_matrix_plotter.plot_comparison_grid(
                experiments_data=all_results, output_dir=output_dir
            )
    ...
    def _save_summary_report(self, aggregated: Dict, elapsed_time: float):
        ...
        json_path = os.path.join(output_dir, self.config.output.files.summary_report)
...
def _run_prediction_worker(
    experiment: DictConfig, gpu_id: int, dataset_csv: str, config: DictConfig
) -> Dict:
    """
