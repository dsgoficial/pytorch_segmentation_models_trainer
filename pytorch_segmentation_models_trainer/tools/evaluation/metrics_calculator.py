...
def _get_spatial_overlap(pred_path: str, gt_path: str) -> tuple:
    ...
    with rasterio.open(pred_path) as pred_src, rasterio.open(gt_path) as gt_src:
        ...
        if (
            ...
            gt_window = Window(
                gt_window.col_off, gt_window.row_off, matched_shape[1], matched_shape[0]
            )
...
class MetricsCalculator:
    ...
    def _instantiate_metrics(self) -> List:
        ...
        for idx, metric_cfg in enumerate(self.config.metrics.segmentation_metrics):
            try:
                ...
                metric_json = metric_json.replace(
                    "${metrics.num_classes}", str(self.num_classes)
                )
                metric_json = metric_json.replace(
                    "${hyperparameters.num_classes}", str(self.num_classes)
                )
                ...
                logger.error(f"Failed to instantiate metric {idx}: {e}", exc_info=True)
    ...
    def _get_spatial_overlap(self, pred_path: str, gt_path: str) -> tuple:
        return _get_spatial_overlap(pred_path, gt_path)
    ...
    def calculate_metrics(
        ...
        results_dict = self._compute_metrics_from_results(
            image_results, experiment_name
        )
    ...
    def _process_images_sequential(self, tasks: list) -> list:
        ...
        for task in tqdm(
            tasks, desc=f"Evaluating {self.experiment_config.name}", unit="img"
        ):
            try:
    ...
    def _compute_metrics_from_results(
        ...
        for result in tqdm(image_results, desc="Computing metrics", unit="img"):
            ...
            image_metrics = self._metrics_from_confusion_matrix(
                cm_per_image, image_name
            )
    ...
    def _load_prediction_flexible(
        self, predictions_folder: str, row: pd.Series, prediction_map: Dict[str, str]
    ) -> tuple:
        """
        ...
        for variant in gt_variants:
            if variant in prediction_map:
                ...
                logger.debug(
                    f"Matched {gt_stem} -> {Path(matched_path).name} (variant: {variant})"
                )
                break
    ...
    def _calculate_per_image_metrics(
        ...
        assert pred_flat.dtype in [
            torch.int32,
            torch.int64,
        ], f"pred_flat deve ser int, mas é {pred_flat.dtype}"
        assert gt_flat.dtype in [
            torch.int32,
            torch.int64,
        ], f"gt_flat deve ser int, mas é {gt_flat.dtype}"
        assert (
            pred_flat.min() >= 0
        ), f"pred_flat tem valores negativos: {pred_flat.min()}"
        assert gt_flat.min() >= 0, f"gt_flat tem valores negativos: {gt_flat.min()}"
        assert (
            pred_flat.max() < self.num_classes
        ), f"pred_flat tem valores >= num_classes: {pred_flat.max()}"
        assert (
            gt_flat.max() < self.num_classes
        ), f"gt_flat tem valores >= num_classes: {gt_flat.max()}"
    ...
    def _get_output_dir(self, experiment_name: str) -> str:
        ...
        if (
            hasattr(self.config.output, "timestamp_folders")
            and self.config.output.timestamp_folders
        ):
        ...
        if hasattr(self.config.output, "structure"):
            ...
            output_dir = os.path.join(base_dir, experiment_name, "metrics")
...
def _process_single_image_worker(
    task: Dict, num_classes: int, class_names: List[str]
) -> Dict:
    """
...
def _read_aligned_rasters_worker(
    ...
    try:
        ...
        overlap_result = _get_spatial_overlap(pred_path, gt_path)
        ...
        if np.isnan(pred_array).any():
            logger.warning(
                f"NaN values found in prediction {pred_path}, replacing with 0"
            )
            pred_array = np.nan_to_num(pred_array, nan=0.0).astype(np.int32)

        if np.isnan(gt_array).any():
            logger.warning(
                f"NaN values found in ground truth {gt_path}, replacing with 0"
            )
