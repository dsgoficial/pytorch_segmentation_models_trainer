...
class DatasetCSVBuilder:
    ...
    def build_csv(self, output_path: str) -> pd.DataFrame:
        ...
        logger.info(
            f"Unique dimensions: {df[['width', 'height']].drop_duplicates().shape[0]}"
        )
    ...
    def _match_custom_regex(self, image_path: Path) -> Optional[Path]:
        ...
        if not regex_pattern:
            raise ValueError("custom_regex strategy requires 'regex_pattern' in config")
    ...
    def validate_dataset(self, df: pd.DataFrame) -> bool:
        ...
        if len(invalid_dims) > 0:
            logger.warning(f"{len(invalid_dims)} images with invalid dimensions (0x0)")

        # 4. Verificar duplicatas
        duplicates = df[df.duplicated(subset=["image"], keep=False)]
        if len(duplicates) > 0:
            logger.warning(f"{len(duplicates)} duplicate images found")
