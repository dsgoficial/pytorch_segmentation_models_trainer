...
class ResultsAggregator:
        ...
        for exp_name, results in all_results.items():
            ...
            summary["experiments_summary"][exp_name] = {
                ...
                "key_metrics": {
                    ...
                    if not k.endswith("_class_0")
                    and not k.endswith("_class_1")
    ...
    def _create_rankings(self, all_results: Dict) -> Dict:
        ...
        for metric_name in metric_names:
            ...
            for exp_name, results in all_results.items():
                if (
                    results
                    and "aggregated" in results
                    and metric_name in results["aggregated"]
                ):
    ...
    def _calculate_statistics(self, all_results: Dict) -> Dict:
        ...
        for exp_name, results in all_results.items():
            ...
            metric_cols = [
                ...
                if col != "image"
                and per_image_df[col].dtype in [np.float64, np.float32, np.int64]
            ]
    ...
    def _find_best_experiment(
        self, all_results: Dict, metric: str = "JaccardIndex"
    ) -> Dict:
        """
        ...
        if best_exp:
            return {"name": best_exp, "metric": metric, "score": best_score}
    ...
    def _find_worst_experiment(
        ...
        if worst_exp:
            return {"name": worst_exp, "metric": metric, "score": worst_score}
    ...
    def _save_aggregated_csv(self, all_results: Dict):
        ...
        if len(all_results) == 0:
            ...
            output_path = os.path.join(
                self.config.output.base_dir, "aggregated_metrics.csv"
            )
    ...
    def _save_rankings_csv(self, rankings: Dict):
        ...
        for metric, ranking in rankings.items():
            ...
            output_dir = os.path.join(
                self.config.output.base_dir, self.config.output.structure.reports_folder
            )
            Path(output_dir).mkdir(parents=True, exist_ok=True)

            csv_path = os.path.join(output_dir, f"ranking_{metric}.csv")
    ...
    def create_comparison_table(
        self, all_results: Dict, metrics: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
    ...
    def get_improvement_matrix(
        self, all_results: Dict, baseline_exp: str, metric: str = "JaccardIndex"
    ) -> Dict:
        """
        ...
        for exp_name, results in all_results.items():
            ...
            relative_improvement = (
                absolute_improvement / baseline_score if baseline_score != 0 else 0
            )
