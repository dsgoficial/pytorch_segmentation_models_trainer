...
class ConfusionMatrixPlotter:
    ...
    def plot_single_experiment(
        ...
        output_path = os.path.join(
            output_dir, f"confusion_matrix_{experiment_name}.{self.save_format}"
        )
    ...
    def plot_comparison_grid(
        ...
        """
        logger.info(
            f"Plotting confusion matrix comparison grid for {len(experiments_data)} experiments"
        )
    ...
    def plot_difference_matrix(
        self, experiment1_data: Dict, experiment2_data: Dict, output_dir: str
    ) -> str:
        """
    ...
    def plot_per_class_accuracy(
        ...
        """
        logger.info(
            f"Plotting per-class accuracy for {len(experiments_data)} experiments"
        )
        ...
        # Salvar
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        output_path = os.path.join(output_dir, f"per_class_accuracy.{self.save_format}")
        plt.savefig(output_path, dpi=self.dpi, bbox_inches="tight")
        plt.close()
