...
class ComparisonPlotter:
    ...
    def plot_metrics_bar_chart(
        self, experiments_data: Dict[str, Dict], output_dir: str
    ) -> str:
        """
        ...
        """
        logger.info(
            f"Plotting metrics bar chart for {len(experiments_data)} experiments"
        )
        ...
        for i, (metric, values) in enumerate(metrics_data.items()):
            ...
            bars = ax.bar(
                x + offset, values, width, label=metric, alpha=0.8, color=colors[i]
            )
        ...
        ax.set_title(
            "Metrics Comparison Across Experiments", fontsize=16, fontweight="bold"
        )
        ax.set_xticks(x)
        ...
        output_path = os.path.join(output_dir, f"metrics_bar_chart.{self.save_format}")
    ...
    def plot_per_image_boxplot(
        ...
        for exp_name, data in experiments_data.items():
            ...
            if metric in per_image_df.columns:
                ...
                for value in values:
                    plot_data.append({"Experiment": exp_name, "Score": value})
        ...
        # Box plot
        sns.boxplot(data=df, x="Experiment", y="Score", ax=ax, palette="Set3")

        # Adicionar pontos individuais (strip plot)
        sns.stripplot(
            data=df, x="Experiment", y="Score", ax=ax, color="black", alpha=0.3, size=3
        )
        ...
        ax.set_title(
            f"{metric} Distribution Across Images", fontsize=16, fontweight="bold"
        )
    ...
    def plot_radar_chart(
        ...
        # Título e legenda
        plt.title(
            "Metrics Comparison - Radar Chart", fontsize=16, fontweight="bold", pad=20
        )
        ...
        # Salvar
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        output_path = os.path.join(output_dir, f"radar_chart.{self.save_format}")
        plt.savefig(output_path, dpi=self.dpi, bbox_inches="tight")
        plt.close()
    ...
    def plot_best_worst_images(
        ...
        for exp_name, data in experiments_data.items():
            ...
            ax1.set_yticklabels(
                [Path(img).name for img in worst["image"].values], fontsize=9
            )
            ax1.set_xlabel(metric, fontsize=12, fontweight="bold")
            ax1.set_title(
                f"Worst {n_images} Images",
                fontsize=14,
                fontweight="bold",
                color="#ff6b6b",
            )
            ...
            ax2.set_yticklabels(
                [Path(img).name for img in best["image"].values], fontsize=9
            )
            ax2.set_xlabel(metric, fontsize=12, fontweight="bold")
            ax2.set_title(
                f"Best {n_images} Images",
                fontsize=14,
                fontweight="bold",
                color="#51cf66",
            )
            ...
            output_path = os.path.join(
                exp_output_dir, f"best_worst_analysis.{self.save_format}"
            )
            plt.savefig(output_path, dpi=self.dpi, bbox_inches="tight")
            plt.close()
    ...
    def plot_metrics_distribution(
        ...
        for exp_name, data in experiments_data.items():
            ...
            if metric in per_image_df.columns:
                ...
                # Histogram + KDE
                sns.histplot(values, kde=True, label=exp_name, alpha=0.4, ax=ax)

        # Configurar
        ax.set_xlabel(metric, fontsize=14, fontweight="bold")
        ax.set_ylabel("Frequency", fontsize=14, fontweight="bold")
        ax.set_title(
            f"{metric} Distribution Comparison", fontsize=16, fontweight="bold"
        )
        ax.legend()
        ax.grid(axis="y", alpha=0.3)
        ...
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        output_path = os.path.join(
            output_dir, f"distribution_{metric}.{self.save_format}"
        )
        plt.savefig(output_path, dpi=self.dpi, bbox_inches="tight")
        plt.close()
    ...
    def plot_per_class_comparison(
        ...
        per_class_metrics = [
            key for key in aggregated.keys() if key.startswith(metric_prefix + "_")
        ]
        ...
        class_names_from_metrics = [
            metric.replace(metric_prefix + "_", "") for metric in per_class_metrics
        ]
        ...
        for i, (exp_name, values) in enumerate(zip(exp_names, values_matrix)):
            ...
            ax.bar(
                x + offset, values, width, label=exp_name, alpha=0.8, color=colors[i]
            )
        ...
        ax.set_title(
            f"{metric_prefix} Per-Class Comparison", fontsize=16, fontweight="bold"
        )
        ax.set_xticks(x)
        ...
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        output_path = os.path.join(
            output_dir, f"per_class_{metric_prefix}.{self.save_format}"
        )
        plt.savefig(output_path, dpi=self.dpi, bbox_inches="tight")
        plt.close()
