...
def shapely_postprocess(polylines, np_indicator, tolerance, config):
    ...
    list(
        map(
            ...
            shapely.ops.polygonize(shapely.ops.unary_union(linestring_list)),
        )
    )
...
def _get_linestring_list(polylines, width, height, tol):
    line_string_iter = (
        shapely.geometry.LineString(polyline[:, ::-1]) for polyline in polylines
    )
    line_string_list = [
...
def post_process(polylines, np_indicator, np_crossfield, config):
    ...
    polygons, probs = shapely_postprocess(
        polylines, np_indicator, config.tolerance, config
    )
...
def get_marching_squares_skeleton(np_int_prob, config):
    ...
    contours = [
        ...
        and shapely.geometry.Polygon(contour).area > config.min_area
    ]
    ...
    return Skeleton(
        ...
        paths=Paths(indices=np.array(indices), indptr=np.array(indptr)),
...
def compute_skeletons(seg_batch, config, spatial_gradient, pool=None) -> List[Skeleton]:
    ...
    return (
        _compute_skeletons_with_marching_squares(seg_batch, config, pool)
        if config.init_method == "marching_squares"
        else _compute_seletons_with_skeletonize(
            seg_batch, config, spatial_gradient, pool
        )
    )
...
def _compute_seletons_with_skeletonize(seg_batch, config, spatial_gradient, pool):
    ...
    corrected_edge_prob_batch = (
        2 * spatial_gradient(corrected_edge_prob_batch)[:, 0, :, :]
    )
    ...
    skeletons_batch = (
        pool.map(get_skeleton_partial, np_corrected_edge_mask_batch)
        if pool is not None
...
def _compute_skeletons_with_marching_squares(seg_batch, config, pool):
    ...
    get_marching_squares_skeleton_partial = functools.partial(
        get_marching_squares_skeleton, config=config
    )
    return (
        pool.map(get_marching_squares_skeleton_partial, np_int_prob_batch)
        if pool is not None
...
class PolygonizerASM:
    ...
    def __call__(self, seg_batch, crossfield_batch, pre_computed=None):
        assert (
            len(seg_batch.shape) == 4 and seg_batch.shape[1] <= 3
        ), "seg_batch should be (N, C, H, W) with C <= 3, not {}".format(
            seg_batch.shape
        )
        assert (
            len(crossfield_batch.shape) == 4 and crossfield_batch.shape[1] == 4
        ...
        tensorskeleton = skeletons_to_tensorskeleton(
            skeletons_batch, device=self.config.device
        )
        ...
        tensorskeleton_optimizer = TensorSkeletonOptimizer(
            self.config, tensorskeleton, int_prob_batch, crossfield_batch
        )
    ...
    def _skeletons_to_polygons(
        self, crossfield_batch, int_prob_batch, out_skeletons_batch
    ):
        polylines_batch = [
