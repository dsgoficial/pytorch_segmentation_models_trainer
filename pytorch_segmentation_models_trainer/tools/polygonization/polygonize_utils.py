...
def compute_init_contours(np_indicator, level):
    assert (
        isinstance(np_indicator, np.ndarray) and len(np_indicator.shape) == 2
    ), "indicator should have shape (H, W)"
    # Using marching squares
    contours = skimage.measure.find_contours(
        np_indicator, level, fully_connected="low", positive_orientation="high"
    )
    return contours
...
def compute_init_contours_batch(np_indicator_batch, level, pool=None):
    ...
    return (
        pool.map(post_process_partial, np_indicator_batch)
        if pool is not None
        else list(map(post_process_partial, np_indicator_batch))
    )
...
def split_polylines_corner(polylines, corner_masks):
    ...
    for polyline, corner_mask in zip(polylines, corner_masks):
        ...
        if (
            ~corner_mask[0] and ~corner_mask[-1]
        ):  # In fact any of those conditon should be enough
            new_polyline = np.concatenate(
                [polyline[splits[-1] :], polyline[: splits[0] + 1]], axis=0
            )
