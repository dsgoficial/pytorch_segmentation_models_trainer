...
class RasterFile:
    ...
    def read_as_numpy_array(self):
        ...
        return numpy_array if not self.channels_last else reshape_as_image(numpy_array)
    ...
    def export_to(self, output_dir: Path, output_format: str, output_basename=None):
        with rasterio.open(self.file_name) as src:
            ...
            adjusted_filename = (
                Path(self.file_name).with_suffix(suffix_dict[output_format]).name
                if output_basename is None
                else output_basename + suffix_dict[output_format]
            )
    ...
    def build_mask(
        self,
        input_vector_layer: GeoDF,
        output_dir: Path,
        output_dir_dict: OrderedDict,
        ...
        output_extension = (
            os.path.basename(self.file_name).split(".")[-1]
        ...
        mask_feats = input_vector_layer.get_features_from_bbox(
            raster_ds.bounds.left,
            raster_ds.bounds.right,
            raster_ds.bounds.bottom,
            raster_ds.bounds.top,
    ...
    def write_masks_to_disk(
        ...
        subfolders = (
            str(self.file_name.parents[0]).split(folder_basename)[-1]
            if replicate_input_structure
            else ""
        )
        ...
        for key, path in output_dir_dict.items():
            ...
            if key == "bounding_boxes":
                ...
                continue
            profile["count"] = (
                1 if len(raster_dict[key].shape) == 2 else min(raster_dict[key].shape)
            )
            output = os.path.join(
                path, subfolders, output_filename + "." + output_extension
            )
...
class CSVImageReaderProcessor(AbstractRasterPathListGetter):
    ...
    def read_csv(self) -> pd.DataFrame:
        return (
            pd.read_csv(self.input_csv_path)
            if self.n_first_rows_to_read is None
...
def save_with_rasterio(output, profile, raster_iter, mask_types):
    raster_array = (
        list(raster_iter)[0] if len(mask_types) == 1 else np.dstack(raster_iter)
    )
    with rasterio.open(output, "w", **profile) as out:
