...
class _ObjectAttentionBlock(nn.Module):
    ...
    def __init__(
        self, in_channels, key_channels, scale=1, bn_type=None, align_corners=True
    ):
...
class SpatialOCR_Module(nn.Module):
    ...
    def __init__(
        ...
        self.object_context_block = ObjectAttentionBlock2D(
            in_channels, key_channels, scale, bn_type, align_corners=align_corners
        )
...
class HRNetOCR(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.ocr = (
            OmegaConf.create(
                {"mid_channels": 512, "key_channels": 256, "dropout": 0.05, "scale": 1}
            )
            if ocr is None
            else ocr
        )
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1):
        ...
        layers += [block(inplanes, planes) for _ in range(1, blocks)]
        return nn.Sequential(*layers)
    ...
    def _make_stage(self, layer_config, num_inchannels, multi_scale_output=True):
        ...
        for i in range(num_modules):
            ...
            reset_multi_scale_output = (
                False if not multi_scale_output and i == num_modules - 1 else True
            )
            modules.append(
