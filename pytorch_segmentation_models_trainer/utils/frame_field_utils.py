...
class LaplacianPenalty:
    def __init__(self, channels: int):
        ...
        self.filter = (
            torch.tensor([[0.5, 1.0, 0.5], [1.0, -6.0, 1.0], [0.5, 1.0, 0.5]]) / 12
        )
        self.filter = self.filter[None, None, ...].expand(self.channels, -1, -1, -1)

    def laplacian_filter(self, tensor):
        penalty_tensor = F.conv2d(
            tensor, self.filter.to(tensor.device), padding=1, groups=self.channels
        )
...
def framefield_align_error(c0, c2, z, complex_dim=-1):
    ...
    assert (
        c0.shape[complex_dim] == c2.shape[complex_dim] == z.shape[complex_dim] == 2
    ...
    for polyline in polylines:
        ...
        if np.max(np.abs(polyline[0] - polyline[-1])) < 1e-6:
            ...
            left_edges = np.concatenate(
                [polyline[-2:-1] - polyline[-1:], polyline[:-2] - polyline[1:-1]],
                axis=0,
            )
...
def compute_crossfield_to_plot(gt_crossfield_angle, crossfield_shape=None):
    if torch.is_tensor(gt_crossfield_angle):
        u_angle = gt_crossfield_angle.detach().cpu().numpy()
    else:
        u_angle = np.array(gt_crossfield_angle)
    ...
    if crossfield_shape is None:
        crossfield = torch.zeros((1, 4, u_angle.shape[-2], u_angle.shape[-1]))
    else:
        crossfield = torch.zeros(crossfield_shape)

    crossfield[:, 0, :, :] = torch.from_numpy(np.atleast_1d(c0.real).astype(np.float32))
    crossfield[:, 1, :, :] = torch.from_numpy(np.atleast_1d(c0.imag).astype(np.float32))
    crossfield[:, 2, :, :] = torch.from_numpy(np.atleast_1d(c2.real).astype(np.float32))
    crossfield[:, 3, :, :] = torch.from_numpy(np.atleast_1d(c2.imag).astype(np.float32))
    return crossfield
