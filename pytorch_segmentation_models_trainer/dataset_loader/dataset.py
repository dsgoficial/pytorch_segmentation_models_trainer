...
class AbstractDataset(Dataset):
    ...
    def load_image(
        self, idx, key=None, is_mask=False, force_rgb=False, is_binary_mask=True
    ):
        key = self.image_key if key is None else key
    ...
    def load_image_from_path(
        self, image_path, is_mask=False, force_rgb=False, is_binary_mask=True
    ):
        try:
            image = (
...
class SegmentationDataset(AbstractDataset):
    ...
    Args:
        input_csv_path (Path): Path to the CSV file with the data. Mutually
            exclusive with ``df``; one of the two must be provided.
        df (Optional[pd.DataFrame]): Pre-built DataFrame with ``image`` and
            ``mask`` columns. Allows creating the dataset without a CSV on disk
            (e.g. via :class:`SegmentationDatasetFromFolder`).
...
class SegmentationDatasetFromFolder(SegmentationDataset):
    ...
    def __init__(
        ...
        df = self._build_dataframe(
            image_folder, mask_folder, image_extension, mask_extension
        )
...
@contextlib.contextmanager
def _rasterio_read_lock(
    path: str,
    enabled: bool = False,
    lock_dir: Optional[Union[str, Path]] = None,
):
    """Serialize rasterio reads for one source path across DataLoader workers.

    Args:
        path: Raster path used to derive a stable per-file lock name.
        enabled: When ``True``, acquire an exclusive OS-level advisory lock
            before yielding.
        lock_dir: Directory used to store lock files. Defaults to
            ``/tmp/psmt_rasterio_locks``.

    Yields:
        None. Code inside the context runs while holding the lock when
        enabled.
    """
    ...
    root = Path(lock_dir or "/tmp/psmt_rasterio_locks")
    root.mkdir(parents=True, exist_ok=True)
    resolved_path = str(Path(path).resolve()).encode("utf-8")
    key = hashlib.sha1(resolved_path).hexdigest()
...
class RandomCropSegmentationDataset(AbstractDataset):
    ...
    Args:
        ...
        serialize_rasterio_reads: If True, wraps rasterio window reads in a
            per-file interprocess lock. This avoids concurrent reads from the
    ...
    def __init__(
        ...
        self._pre_mix_transform = (
            load_augmentation_object(pre_mix_color_aug) if pre_mix_color_aug else None
        )
    ...
    def _finalize_class_position_index(
        self, class_positions: dict, label: str = "built"
    ):
        ...
        counts = np.array(
            [len(self._class_positions.get(c, [])) for c in range(self.n_classes)],
            dtype=np.float64,
        )
    ...
    def _build_grid_positions(self):
        ...
        build_class_tiles = bool(self.cutmix_rare_classes) or bool(
            getattr(self, "cutmix_prob", 0)
        )

        if build_class_positions:
            class_positions = {c: [] for c in range(self.n_classes)}
        class_tiles = (
            {c: [] for c in range(self.n_classes)} if build_class_tiles else None
        )
        ...
        if build_class_positions:
            self._finalize_class_position_index(
                class_positions, label="(from grid pass)"
            )
    ...
    def _save_grid_cache(self):
        ...
        with open(self.grid_cache, "w") as f:
            json.dump(cache, f)
        logger.info(
            f"Grid cache saved: {self.grid_cache} ({len(self._grid_positions)} tiles)"
        )
    ...
    def _load_class_presence_cache(self):
        ...
        logger.info(f"Loading class-presence cache from {self.class_presence_cache}...")
        ...
        if unmatched > 0:
            ...
            if ratio < 0.9:
                ...
                return self._build_class_presence_index_fresh()
        logger.info(
            f"Class-presence cache loaded: {matched}/{n_total} " f"images matched"
        )
        for c in range(self.n_classes):
            n_imgs = len(self._class_to_images.get(c, []))
            logger.info(f"  Class {c}: present in {n_imgs} images")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if (
            getattr(self, "cutmix_prob", 0.0) > 0
            and np.random.random() < self.cutmix_prob
        ):
