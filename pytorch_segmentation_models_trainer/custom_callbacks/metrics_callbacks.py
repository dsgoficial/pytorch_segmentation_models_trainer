...
class ClassificationReportCallback(pl.callbacks.Callback):
    ...
    def on_validation_batch_end(
        ...
        assert (
            predictions_flat.shape == targets_flat.shape
    ...
    def on_validation_epoch_end(
        ...
        if len(self.val_predictions) != len(self.val_targets):
            print(
                f"WARNING: Prediction count ({len(self.val_predictions)}) != "
        ...
        try:
            ...
            if self.save_outputs:
                timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
                filename = (
                    f"classification_report_epoch_{current_epoch:03d}_{timestamp}.txt"
                )
                filepath = os.path.join(self.output_path, filename)
