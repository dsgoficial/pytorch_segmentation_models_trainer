...
class LossParamsConfig:
    ...
    junction_angle_weights: List[float] = field(
        default_factory=lambda: [1, 0.01, 0.1, 0.01]
    )
...
cs.store(
    ...
    name="single_image",
    node=SingleImageInfereceProcessorConfig,
)
cs.store(
    group="inference_processor",
    name="frame_field",
    node=SingleImageFromFrameFieldProcessorConfig,
)
cs.store(
    group="inference_processor",
    name="multiclass",
    node=MultiClassInferenceProcessorConfig,
)
cs.store(
    group="inference_processor",
    name="object_detection",
    node=ObjectDetectionInferenceProcessorConfig,
)
cs.store(
    group="inference_processor",
    name="polygon_rnn",
    node=PolygonRNNInferenceProcessorConfig,
)
...
cs.store(
    group="data_writer", name="vector_database", node=VectorDatabaseDataWriterConfig
)

# Export Strategies
cs.store(
    group="export_strategy", name="raster", node=RasterExportInferenceStrategyConfig
)
cs.store(
    group="export_strategy",
    name="multiple_raster",
    node=MultipleRasterExportInferenceStrategyConfig,
)
cs.store(
    group="export_strategy",
    name="vector_file",
    node=VectorFileExportInferenceStrategyConfig,
)
cs.store(
    group="export_strategy",
    name="vector_database",
    node=VectorDatabaseExportInferenceStrategyConfig,
)
cs.store(
    group="export_strategy",
    name="object_detection",
    node=ObjectDetectionExportInferenceStrategyConfig,
)
...
cs.store(
    group="polygonizer", name="polygon_rnn", node=PolygonRNNPolygonizerProcessorConfig
)
