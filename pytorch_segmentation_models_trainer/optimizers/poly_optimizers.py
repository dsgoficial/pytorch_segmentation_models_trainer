    ...
    def __init__(
        ...
        def lr_warmup_func(iter):
            return (
                ...
                + (config.warmup_factor - 1)
                * (config.warmup_iters - iter)
                / config.warmup_iters
            )

        self.lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
            self.optimizer, lr_lambda=lr_warmup_func
        )
...
class PolygonAlignLoss:
    ...
    def __call__(self, tensorpoly):
        ...
        pos_indicator_value = bilinear_interpolate(
        ...
        if self.dist is not None:
            pos_dist_value = bilinear_interpolate(
        ...
        total_loss = (
            self.data_coef * level_loss
            + self.length_coef * length_penalty
            + self.crossfield_coef * total_align_loss
        )
...
class TensorSkeletonOptimizer:
    def __init__(
        ...
        assert (
            len(indicator.shape) == 3
        ), f"indicator should be of shape (N, H, W), not {indicator.shape}"
        assert (
            len(c0c2.shape) == 4 and c0c2.shape[1] == 4
        ), f"c0c2 should be of shape (N, 4, H, W), not {c0c2.shape}"
