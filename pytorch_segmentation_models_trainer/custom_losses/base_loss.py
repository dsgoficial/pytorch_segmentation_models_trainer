...
class Loss(torch.nn.Module):
    ...
    def reset_norm(self):
        self.norm_meter = math_utils.AverageMeter(
            "{}_norm".format(self.name), init_val=1
        )
    ...
    def __repr__(self):
        return "{} (name={}, norm={:0.06})".format(
            self.__class__.__name__, self.name, self.norm[0]
        )
...
class SegLoss(Loss):
    ...
    def compute(self, pred_batch: torch.Tensor, gt_batch: torch.Tensor):
        ...
        mean_focal = (
            0
            if not isinstance(focal_tversky, torch.Tensor) and focal_tversky == 0
            else torch.mean(focal_tversky)
        )
        return (
...
class SegCrossfieldLoss(Loss):
    ...
    def compute(self, pred_batch, gt_batch):
        c0, c2 = pred_batch["crossfield"][:, :2], pred_batch["crossfield"][:, 2:]
        seg_slice_grads_normed = pred_batch["seg_grads_normed"][
            :, self.pred_channel, ...
        ]
        ...
        self.extra_info["seg_slice_grads"] = pred_batch["seg_grads"][
            :, self.pred_channel, ...
        ]
        return avg_align_loss
...
class ComputeSegGrads:
    ...
    def __call__(self, pred_batch, gt_batch):
        ...
        pred_batch["seg_grad_norm"] = pred_batch["seg_grads"].norm(
            dim=2
        )  # (b, c, h, w)
...
def compute_seg_loss_weigths(pred_batch, gt_batch, cfg):
    ...
    pixel_class_freq = (
        gt_polygons_mask * gt_batch["class_freq"][:, :, None, None]
    ...
    if pixel_class_freq.min() == 0:
        ...
        for i, path in enumerate(gt_batch["path"]):
            logger.error(
                f"{i} idx: {gt_batch['idx'][i]}, path: {path}, class_freq: {gt_batch['class_freq'][i]}"
            )
...
def compute_gt_field(pred_batch, gt_batch):
    ...
    gt_field = torch.cat(
        [torch.cos(gt_crossfield_angle), torch.sin(gt_crossfield_angle)], dim=1
    )
...
def build_combined_loss(cfg):
    ...
    if cfg.compute_seg:
        ...
        loss_funcs.append(
            SegLoss(
                ...
                use_mixup=False
                if "use_mixup" not in cfg.pl_trainer
                else cfg.pl_trainer.use_mixup,
                mixup_alpha=0.5
                if "mixup_alpha" not in cfg.pl_trainer
                else cfg.pl_trainer.mixup_alpha,
                use_label_smooth=False
                if "use_label_smoothing" not in cfg.pl_trainer
                else cfg.pl_trainer.use_label_smoothing,
                smooth_factor=0.1
                if "smooth_factor" not in cfg.pl_trainer
    ...
    if cfg.compute_crossfield:
        ...
        weights.append(cfg.loss_params.multiloss.coefs.crossfield_align)
        loss_funcs.append(CrossfieldAlign90Loss(name="crossfield_align90"))
        weights.append(cfg.loss_params.multiloss.coefs.crossfield_align90)
        loss_funcs.append(CrossfieldSmoothLoss(name="crossfield_smooth"))
        weights.append(cfg.loss_params.multiloss.coefs.crossfield_smooth)
    ...
    if cfg.compute_seg:
        ...
        if cfg.seg_params.compute_interior and cfg.compute_crossfield:
            ...
            loss_funcs.append(
                SegCrossfieldLoss(
                    name="seg_interior_crossfield", pred_channel=pred_channel
                )
            )
            weights.append(cfg.loss_params.multiloss.coefs.seg_interior_crossfield)
