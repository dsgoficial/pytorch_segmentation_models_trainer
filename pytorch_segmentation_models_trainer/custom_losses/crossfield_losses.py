...
class AlignLoss:
    def __init__(
        ...
        self.corner_angles = (
            np.pi * torch.tensor(loss_params.corner_angles) / 180
        )  # Convert to radians
        self.corner_angle_threshold = (
            np.pi * loss_params.corner_angle_threshold / 180
        )  # Convert to radians
        self.junction_angles = (
            np.pi * torch.tensor(loss_params.junction_angles) / 180
        )  # Convert to radians
        self.junction_angle_weights = torch.tensor(loss_params.junction_angle_weights)
        self.junction_angle_threshold = (
            np.pi * loss_params.junction_angle_threshold / 180
        )  # Convert to radians
    ...
    def _compute_edge_mask(self, tangents, path_pos):
        ...
        midpoints_batch = self.tensorskeleton.batch[
            self.tensorskeleton.path_index[:-1]
        ]  # Same as start point of edge

        midpoints_int = midpoints.round().long()
        midpoints_int[:, 0] = torch.clamp(
            midpoints_int[:, 0], 0, self.c0c2.shape[2] - 1
        )
        midpoints_int[:, 1] = torch.clamp(
            midpoints_int[:, 1], 0, self.c0c2.shape[3] - 1
        )
        midpoints_c0 = self.c0c2[
            midpoints_batch, :2, midpoints_int[:, 0], midpoints_int[:, 1]
        ]
        midpoints_c2 = self.c0c2[
            midpoints_batch, 2:, midpoints_int[:, 0], midpoints_int[:, 1]
        ]
        ...
        align_loss = frame_field_utils.framefield_align_error(
            midpoints_c0, midpoints_c2, normed_tangents, complex_dim=1
        )
    ...
    def _detect_corners(self, middle_pos, path_batch, prev_tangent, next_tangent):
        with torch.no_grad():
            ...
            middle_pos_int[:, 0] = torch.clamp(
                middle_pos_int[:, 0], 0, self.c0c2.shape[2] - 1
            )
            middle_pos_int[:, 1] = torch.clamp(
                middle_pos_int[:, 1], 0, self.c0c2.shape[3] - 1
            )
            middle_batch = path_batch[1:-1]
            middle_c0c2 = self.c0c2[
                middle_batch, :, middle_pos_int[:, 0], middle_pos_int[:, 1]
            ]
            middle_uv = frame_field_utils.c0c2_to_uv(middle_c0c2)
            prev_tangent_closest_in_uv = frame_field_utils.compute_closest_in_uv(
                prev_tangent, middle_uv
            )
            next_tangent_closest_in_uv = frame_field_utils.compute_closest_in_uv(
            ...
            sub_path_delim, sub_path_sort_indices = torch.sort(
                torch.cat([self.tensorskeleton.path_delim, is_corner_index])
            )
            sub_path_delim_is_corner = (
                self.tensorskeleton.path_delim.shape[0] <= sub_path_sort_indices
            )  # If condition is true, then the delimiter is from is_corner_index
    ...
    def _compute_total_loss(
        ...
        return (
            data_coef * level_loss
            + length_coef * total_length_loss
            + crossfield_coef * total_align_loss
            + curvature_coef * total_curvature_loss
            + corner_coef * total_corner_loss
            + junction_coef * total_junction_loss
        )

    def _compute_sub_path_dissimilarity(
        self, sub_path_delim, sub_path_delim_is_corner, path_pos
    ):
        ...
        with torch.no_grad():
            ...
            sub_path_end_index[
                ~sub_path_delim_is_corner[1:]
            ] -= 1  # For non-corner delimitators, have to shift
            ...
            relative_path_pos_projected = (
                relative_path_pos_projected_lengh[:, None] * expanded_sub_path_normal
            )
    ...
    def _compute_vertex_angles_for_curvature_loss(
        self, prev_tangent, next_tangent, prev_norm, next_norm, is_corner_index
    ):
        ...
        corner_angles = vertex_angles[
            is_corner_index - 1
        ]  # -1 because of the shift of vertex_angles relative to path_pos
        return vertex_angles, corner_angles

    def _compute_curvature_loss(
        self,
        vertex_angles,
        sub_path_delim,
        sub_path_delim_is_corner,
        ...
        sub_path_lengths[
            sub_path_delim_is_corner[1:]
        ] += 1  # Fix length of paths split by corners
    ...
    def _compute_corner_loss(self, corner_angles):
        ...
        corner_snap_dist_optim = corner_snap_dist[
            corner_snap_dist < self.corner_angle_threshold
        ]
    ...
    def _compute_junction_corner_loss(self, pos):
        ...
        junction_prev_dir = junction_prev_tangent / (
            torch.norm(junction_prev_tangent, dim=-1)[:, None] + 1e-6
        )
        junction_next_dir = junction_next_tangent / (
            torch.norm(junction_next_tangent, dim=-1)[:, None] + 1e-6
        )
        junction_dot = (
            junction_prev_dir[:, 0] * junction_next_dir[:, 0]
        ...
        self.junction_angle_weights = self.junction_angle_weights.to(
            junction_abs_angles.device
        )
        junction_snap_dist = torch.abs(
            junction_abs_angles[:, None] - self.junction_angles
        )
        junction_snap_dist *= self.junction_angle_weights[
            None, :
        ]  # Apply weights per target angle (as we use the L1 norm, it works applying before the norm)
        junction_snap_dist_optim = junction_snap_dist[
            junction_snap_dist < self.junction_angle_threshold
        ]
...
class PolygonAlignLoss:
    ...
    def __call__(self, tensorpoly):
        ...
        # Align to crossfield
        align_loss = frame_field_utils.framefield_align_error(
            midpoints_c0, midpoints_c2, z, complex_dim=1
        )
        align_loss = align_loss * edge_mask
        ...
        pos_indicator_value = math_utils.bilinear_interpolate(
            self.indicator[:, None, ...], tensorpoly.pos, batch=tensorpoly.batch
        )
        ...
        if self.dist is not None:
            pos_dist_value = math_utils.bilinear_interpolate(
                self.dist[:, None, ...], tensorpoly.pos, batch=tensorpoly.batch
            )
            dist_loss = torch.sum(torch.pow(pos_dist_value, 2))

        length_penalty = torch.sum(
            torch.pow(norms * edge_mask, 2)
        )  # Sum of squared norm to penalise uneven edge lengths
        ...
        total_loss = torch.dot(
            coef_list, [level_loss, length_penalty, total_align_loss]
        )
...
def _build_junction_index(ts: TensorSkeleton) -> torch.tensor:
    junction_edge_index = torch.empty(
        (2 * ts.num_paths, 2), dtype=torch.long, device=ts.path_index.device
    )
...
def _slice_over_junction_index(
    junction_corner_index,
    junction_end_index,
    junction_angle_to_axis,
    grouped_junction_edge_index,
    ...
    for slice_end in junction_end_index:
        ...
        junction_corner_index[slice_start:slice_end, 0] = slice_junction_edge_index[
            :, 1
        ]
        junction_corner_index[slice_start:slice_end, 1] = slice_junction_edge_index[
            :, 0
        ]
        junction_corner_index[slice_start:slice_end, 2] = slice_junction_edge_index[
            :, 1
        ].roll(-1, dims=0)
...
def _compute_angle_to_vertical_axis_at_junction(
    ts: TensorSkeleton, grouped_junction_edge_index
):
    junction_edge = ts.pos.detach()[grouped_junction_edge_index, :]
    junction_tangent = junction_edge[:, 1, :] - junction_edge[:, 0, :]
    return torch.atan2(junction_tangent[:, 1], junction_tangent[:, 0])
