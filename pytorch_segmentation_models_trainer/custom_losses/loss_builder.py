...
class LossWrapper(nn.Module):
    ...
    def forward(
        self, pred_batch, gt_batch, normalize=True
...
def build_compound_loss_from_config(
    compound_loss_cfg: DictConfig, pre_processes: Optional[List] = None
) -> MultiLoss:
    """
    ...
    for idx, loss_weight_cfg in enumerate(compound_loss_cfg.losses):
        ...
        weight_str = (
            f"{weight}" if isinstance(weight, (int, float)) else f"dynamic{weight}"
        )
...
def build_loss_from_config(
    cfg: DictConfig, pre_processes: Optional[List] = None
) -> MultiLoss:
    """
    ...
    if "compound_loss" in cfg.loss_params and cfg.loss_params.compound_loss is not None:
        ...
        return build_compound_loss_from_config(
            cfg.loss_params.compound_loss, pre_processes=pre_processes
        )
