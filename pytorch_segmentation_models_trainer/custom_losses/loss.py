...
def _compute_reverse_ce(pred_probs, targets, valid_mask, num_classes):
    ...
    one_hot = F.one_hot(targets_safe, num_classes).permute(0, 3, 1, 2).float()
    one_hot = torch.clamp(one_hot, min=1e-4, max=1.0)
    rce = (-pred_probs * torch.log(one_hot)).sum(dim=1)  # (B, H, W)
    rce = (rce * valid_mask.float()).sum() / (valid_mask.float().sum() + 1e-6)
    return rce
...
def _compute_jml(probas_masked, label, smooth, jml_classes):
    ...
    if jml_classes == "present":
        ...
        if present.any():
            ...
            return torch.tensor(0.0, device=probas_masked.device, requires_grad=True)
    else:
        return 1.0 - iou.mean()
    ...
    def _lovasz_softmax(self, probas, labels):
        ...
        if self.per_image:
            ...
            for prob, lab in zip(probas, labels):
                ...
                lab_valid = lab[valid]
                losses.append(
                    _lovasz_softmax_flat(prob_valid, lab_valid, self.lovasz_classes)
                )
            if len(losses) == 0:
                return torch.tensor(0.0, device=probas.device, requires_grad=True)
            return torch.stack(losses).mean()
            ...
            return _lovasz_softmax_flat(vprobas, vlabels, self.lovasz_classes)
...
class WeightedJMLSCELoss(nn.Module):
    ...
    def _smooth_one_hot(self, targets, valid_mask):
        ...
        targets_safe = targets.clone()
        targets_safe[~valid_mask] = 0
        one_hot = F.one_hot(targets_safe, C).permute(0, 3, 1, 2).float()  # [B, C, H, W]
...
class GCBLLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        perm = torch.randperm(len(boundary_indices), device=predictions.device)[
            :n_samples
        ]
...
class DualHeadKendallLoss(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            and epoch < self.consistency_warmup_epochs
        ):
        ...
        loss = (
            precision_hard * L_hard
            + self.log_sigma_hard
            + precision_soft * L_soft
            + self.log_sigma_soft
